...
export class UserProfileComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this._subscriptions.add(this._data.getCategory().subscribe((data: DdlCatogoryName[]) => {
      _.forEach(data, d => {
        d.isCheck = false;
      });
    ...
    this._subscriptions.add(this._data.userActivated.subscribe((id: boolean) => {
      if (id === true) {
        this.isFlag = true;
      } else {
        this.isFlag = false;
  ...
  getPageInfo() {
    this._subscriptions.add(this._data.getAllKbArticle().subscribe(
      (data: KBArticles[]) => {
        this.arr = data;
  ...
  onSearchClick(value) {
    this.value = value;
    if (value !== '') {
      this._subscriptions.add(this._data.getArticleBySearch(value).subscribe(
        (data: KBArticles[]) => {
          window.scroll(0, 0);
          this.arr = data;
          this.page = data['pagerInfo'];
  ...
  onFilterCheck(category) {
    ...
    if (this.value !== '') {
      // console.log(this.value);
      this._subscriptions.add(this._data.getfiltersearch(category.categoryId, this.value).subscribe(
        (data: any[]) => {
          // this.arr = data;
  ...
  getAdminArticles() {
    this._subscriptions.add(this._data.getAdminArticles(this.Page).subscribe(
      (data: KBArticles[]) => {
        this.arr = data;
        this.page = data['pagerInfo'];
        this.totalItem = this.page.totalItems;
  ...
  loadPage(event) {
    ...
    if (this.number !== 0) {
      this._subscriptions.add(this._data.getPageByNumber(this.number, this.Categoryid, this.value).subscribe(
        (data: KBArticles[]) => {
          this.arr = data;
          window.scroll(0, 0);
          this.all_articles = this.arr['kbArticles'];
