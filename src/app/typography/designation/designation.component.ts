...
export class DesignationComponent implements OnInit, OnDestroy {
  private _subscritpions = new Subscription();
...
private _subscriptions = new Subscription();
  ...
  getDesig() {
    this._subscritpions.add(this._data.getDesignations().subscribe(
      (data: Designation[]) => {
        this.arrDesig = data;
  ...
  onDesigDelete(id: number) {
    console.log(id);
    this._subscritpions.add(this._data.deleteDesignation(id).subscribe(
      (data: any) => {
  ...
  onFormSubmit(f) {
    ...
    if (test !== undefined) {
      ...
      if (this.selectedDesignationOption === 'Add') {
        ...
        this._subscritpions.add(this._data.addDesignation(req).subscribe(
          (data: any) => {
            console.log(f.value);
            this.messageService.add({
              severity: 'success',
