...
getDocuments() {
    this._subscriptions.add(this._data.getAllDocuments().subscribe((data: Document[]) => {
        this.arrDoc = data;
        this.loading = false;
...
onDocDelete(id: number) {
    this._subscriptions.add(this._data.deleteDocument(id).subscribe(
        (data: any) => {
  ...
  } else
{
  ...
  if (this.selectedDocumentOption == 'Add') {
    ...
    this._subscriptions.add(this._data.addDocuments(req).subscribe((data: any) => {
      this.messageService.add({
        severity: 'success',
        summary: 'Success Message',
