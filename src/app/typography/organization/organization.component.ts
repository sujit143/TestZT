...
export class OrganizationComponent implements OnInit, OnDestroy {
  ...
  getOrg() {
    this._subscriptions.add(this._data.getOrganization().subscribe((data: Organization[]) => {
  ...
  onFormSubmit(f) {
    ...
    if (test !== undefined) {
      ...
      if (this.selectedDepartmentOption === 'add') {
        ...
        console.log(this.id);
        this._subscriptions.add(this._data.addOrganization(f.value).subscribe((data: any) => {
          this.display = false;
          console.log(f.value);
          setTimeout(() => {
